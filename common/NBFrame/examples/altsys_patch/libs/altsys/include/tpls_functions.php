...
function tplsadmin_get_basefilepath( $dirname , $type , $tpl_file )
{
    ...
    $path = tplsadmin_get_NBFrameTemplate($path,  $dirname , $type , $tpl_file );
...
function tplsadmin_get_NBFrameTemplate($path,  $dirname , $type , $tpl_file ) {
    if (file_exists(XOOPS_ROOT_PATH.'/modules/'.$dirname.'/xoops_version.php')) {
        ...
        if ($type == 'module') {
            foreach($modversion['templates'] as $template_info) {
                if (isset($template_info['orig_file'])&&($template_info['file'] == $tpl_file)) {
                    $tpl_file = $template_info['orig_file'];
                    $path = NBFrame::findFile($tpl_file, $environment, 'templates', false);
                }
            }
        } else {
            foreach($modversion['blocks'] as $block_info) {
                if (isset($block_info['orig_template'])&&($block_info['template'] == $tpl_file)) {
                    $tpl_file = $block_info['orig_template'];
                    $path = NBFrame::findFile($tpl_file, $environment, 'templates/blocks', false);
                }
            }
        }
    }
